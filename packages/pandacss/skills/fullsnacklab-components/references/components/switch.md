...
Toggle one immediate setting on or off.

## Import

```tsx
import { Switch } from "@fullsnacklab/components";
```
...
- Components and helpers: `Root`, `RootProvider`, `Label`, `Thumb`, `HiddenInput`, `Control`, `Indicator`, `ThumbIndicator`, `Context`.
- Exported contract types: `RootProps`.

Source defaults:

- Control renders Thumb when no children are supplied.
...
Switch.Root
Switch.HiddenInput
Switch.Control + Thumb + Indicator
Switch.Label
```
