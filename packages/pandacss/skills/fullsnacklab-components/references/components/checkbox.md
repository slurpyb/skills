...
Toggle one boolean choice or a set of independent choices.

## Import

```tsx
import { Checkbox } from "@fullsnacklab/components";
```
...
- Components and helpers: `Root`, `RootProvider`, `Control`, `Group`, `Label`, `HiddenInput`, `Indicator`, `CheckedState`, `GroupProvider`.
- Exported contract types: `RootProps`, `HiddenInputProps`.
