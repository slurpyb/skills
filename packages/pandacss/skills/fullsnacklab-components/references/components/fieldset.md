...
Group related controls under one legend and shared messages.

## Import

```tsx
import { Fieldset } from "@fullsnacklab/components";
```
...
- Components and helpers: `Root`, `RootProvider`, `Legend`, `HelperText`, `ErrorText`, `Content`, `Control`, `Context`.
...
Fieldset.Root
Fieldset.Legend
Fieldset.HelperText or ErrorText
Fieldset.Content
Fieldset.Control
```
...
- Legend names the group; individual controls still need their own labels.
