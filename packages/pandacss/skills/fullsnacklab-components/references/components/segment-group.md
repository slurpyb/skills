...
Choose one segment from a compact set of peers.

## Import

```tsx
import { SegmentGroup } from "@fullsnacklab/components";
```
...
- Components and helpers: `Root`, `RootProvider`, `Indicator`, `Item`, `ItemControl`, `ItemHiddenInput`, `ItemText`, `Label`, `Items`, `Context`.
- Exported contract types: `RootProps`, `ItemsProps`.

Source defaults:

- Root defaults to horizontal orientation.
...
SegmentGroup.Root
SegmentGroup.Label
SegmentGroup.Items or Item repeated
SegmentGroup.ItemHiddenInput
SegmentGroup.ItemControl + ItemText
