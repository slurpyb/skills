...
type BroadTypeKind = "top" | "object" | "record" | "partial";
...
function partialTargetType(type: ESTree.TSType): ESTree.TSType | null {
  const unwrapped = unwrapTypeParentheses(type);
  if (unwrapped.type !== "TSTypeReference") return null;
  if (typeReferenceName(unwrapped) === "Readonly") {
    const [inner] = unwrapped.typeArguments?.params ?? [];
    return inner === undefined ? null : partialTargetType(inner);
  }
  if (typeReferenceName(unwrapped) !== "Partial") return null;
  const [target] = unwrapped.typeArguments?.params ?? [];
  return target ?? null;
}
...
function broadTypeKind(type: ESTree.TSType): BroadTypeKind | null {
  ...
  if (partialTargetType(unwrapped) !== null) return "partial";
...
function widenedBinding(
  ...
  const broadType =
    declaredBroadKind === broadKind
      ? declaredType
      : initializerAssertion?.typeAnnotation;
  const partialTarget =
    broadKind === "partial" && broadType !== undefined
      ? partialTargetType(broadType)
      : null;
  const effectiveEvidence =
    partialTarget === null ? evidence : { type: partialTarget };
  return {
    broadKind,
...
/** Detect locals that erase evidence or accumulate Partial<T> before asserting a complete type. */
export const noWidenThenAssertRule = defineRule({
  meta: {
    type: "problem",
    docs: {
      description:
        "Disallow local const flows that widen known values or accumulate Partial<T> before asserting a narrower or complete type.",
    },
    messages: {
      widenThenAssert:
        'Binding "{{name}}" discards or omits type evidence and later recreates completeness with an assertion. Preserve the precise type, use staged construction, or validate once at the boundary.',
    },
  },
  createOnce(context) {
