...
| `--local-repo-path PATH` | Analyze an already-cloned repo (skips the GitHub API entirely) |
...
| Local clone of a repo | `--local-repo-path PATH` | analyses an already-cloned repo, **bypasses the GitHub API** file limits |
...
`--target` ∈ `claude` (default) | `gemini` | `openai` | `markdown`.

| Target | Archive | Entry file | Use for |
| --- | --- | --- | --- |
| claude | `.zip` | `SKILL.md` (YAML frontmatter) | Claude Code/Desktop/claude.ai |
| gemini | `.tar.gz` | `system_instructions.md` | Google AI Studio (`[gemini]` extra) |
| openai | `.zip` | `assistant_instructions.txt` + vector store | ChatGPT/Assistants (`[openai]` extra) |
...
> In non-interactive/headless use that prompt throws `EOF when reading a line`.
> Workarounds: pipe a confirmation, or skip the gate.
>
> ```bash
> printf 'y\n' | skill-seekers package output/react/ --target claude --no-open
...
| `--clean` | Remove old progress files |
...
> **Dispatch bug:** `skill-seekers workflows <action>` is **broken** — it forwards bad
> args (`--workflows-action`, `--workflow-name`) and errors with "unrecognized
> arguments". **Use the direct entry point `skill-seekers-workflows <action>`**, which
> works.
...
skill-seekers-workflows validate ./my.yaml   # validate by file or by name
```
...
with `--workflow-dry-run`, validate with `skill-seekers-workflows validate ./my-workflow.yaml`.
...
| `skill-seekers install <skill.zip>` | Complete workflow: fetch → scrape → enhance → package → upload |
...
> run `create --config <f> --dry-run` (`estimate --config <f>` still works for sizing).
