...
### It outputs to 16+ targets — not just Claude
- **RAG / vector:** LangChain, LlamaIndex, Chroma, FAISS, Haystack, Qdrant, Weaviate, Pinecone
- **AI platforms:** Claude, Gemini, OpenAI
- **AI coding assistants:** Cursor (`.cursorrules`), Windsurf (`.windsurfrules`), Cline (`.clinerules`), Continue.dev
- **Generic:** Markdown, JSON, YAML
→ "Generate .cursorrules for this framework" and "export this as LangChain documents" are both one `--target`.
...
```
**Note:** the `skill-seekers workflows` subcommand is broken (bad-arg dispatch) — call
the direct entry point `skill-seekers-workflows <action>` instead.
...
- `estimate` / `estimate_job` before a big run to predict pages, time, and cost.
...
- **Generate a config automatically** for an unknown site: MCP `generate_config` or
  `scrape --interactive`.
- **Split a huge skill** and **generate a router/hub skill** over sub-skills
  (`split_skill` / `generate_router`).
...
| Mistake | Reality |
| --- | --- |
| Reaching for `scrape` / `github` / `pdf` / `unified` / `validate` subcommands | **Gone in 3.6.** Every source funnels through **`create`** (positional auto-detect or a flag: `--url`/`--repo`/`--pdf`/`--config`/…). Validate a config with `create --config <f> --dry-run`. |
...
| `skill-seekers workflows list` | **Broken dispatch** — forwards bad args, errors "unrecognized arguments". Use the direct entry point **`skill-seekers-workflows <action>`** (list \| show \| copy \| add \| remove \| validate). |
...
| `package` hangs / `EOF when reading a line` | `package` runs a quality check then prompts `Continue with packaging? (y/n)`. Headless that throws `EOF`. Pipe `printf 'y\n' \| skill-seekers package <dir> --target claude --no-open` or pass `--skip-quality-check`. |
...
- Run `estimate` / `estimate_job` before anything long or potentially billable.
