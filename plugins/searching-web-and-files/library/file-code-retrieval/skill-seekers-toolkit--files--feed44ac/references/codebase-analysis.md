...
# already-cloned repo (bypasses the GitHub API file limits):
skill-seekers create --local-repo-path /tmp/repo -p comprehensive
```

**MCP:** "Analyze this codebase comprehensively", or call `analyze_codebase`
(`{ comprehensive: true }`), then `detect_patterns` / `extract_tests` /
`generate_guides` / `generate_architecture` individually.
...
3. **Issues** — open/closed issues & PRs (when `--include-issues` / `fetch_issues`).
...
- Deep analysis of a large repo can take an hour — run `estimate_job` / `estimate`
  first, and rely on auto-checkpointing (`resume <job-id>`) if interrupted.
- Local enhancement (`--enhance-mode local`) pairs well with deep analysis to turn
  raw extractions into curated guides — for free.
