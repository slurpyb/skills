...
if (hasFlag("--help") || hasFlag("-h")) {
  console.error(
    "[CDP_RUNNER] Usage: node cdp-runner.mjs <script.mjs> [--port 9222] [--new-tab <url>] [--target <id>] [--target-url <pattern>] [--target-type <type>] [--list-targets] [--keep-tab] [--no-stealth]",
  );
  process.exit(0);
}
...
if (nodeMajor < 22) {
  console.error(
    `[CDP_RUNNER] Node.js 22+ required (you have ${process.versions.node}). Native WebSocket is unavailable.`,
  );
  process.exit(1);
}
      ...
      if (msg.id !== undefined && pending.has(msg.id)) {
        ...
        handlers.get(msg.method)?.forEach((h) => {
          try {
            ...
            console.error("[CDP_RUNNER] Handler error:", e.message);
          }
        });
        handlers.get("*")?.forEach((h) => {
          try {
            h(msg.method, msg.params ?? {}, meta);
          } catch {}
        });
      }
    };
...
async function main() {
  ...
  if (NEW_TAB) {
    ...
    const openUrl =
      stealthEnabled() && !isAboutOrDataUrl(tabUrl) ? "about:blank" : tabUrl;
    ...
    const targets = await getTargets();
    const pool = TARGET_TYPE
      ? targets.filter((t) => t.type === TARGET_TYPE)
      : targets;
  ...
  if (runHistory.runs.length > 100)
    runHistory.runs = runHistory.runs.slice(-100);
  ...
  try {
    if (stealthEnabled()) {
      ...
      if (pendingNavigate) {
        console.error(
          `[CDP_RUNNER] Stealth gate: navigating to ${pendingNavigate}`,
        );
        await cdp.send("Page.navigate", { url: pendingNavigate });
        await new Promise((r) => setTimeout(r, 2500));
        if (targetInfo) targetInfo = { ...targetInfo, url: pendingNavigate };
      } else if (
        /^https?:/i.test(targetInfo?.url ?? "") &&
        process.env.CDP_STEALTH_NO_RELOAD !== "1"
      ) {
