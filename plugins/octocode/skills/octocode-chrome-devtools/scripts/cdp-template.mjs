export async function run(cdp) {
  ...
  cdp.addReasoningStep?.({
    ...
    nextAction:
      "Collect signals and decide if targeted follow-up tabs are needed",
  });
  ...
  cdp.on("Runtime.exceptionThrown", ({ exceptionDetails }) => {
    ...
    if (frame)
      console.log(
        `[EXCEPTION_LOCATION] ${frame.url}:${frame.lineNumber}:${frame.columnNumber} in ${frame.functionName || "(anonymous)"}`,
      );
  });
  ...
  cdp.on("Network.responseReceived", ({ requestId, response }) => {
    ...
    console.log(
      `[NETWORK] ${response.status} ${r.method} ${r.url} (${duration}ms)`,
    );
    if (response.status >= 400)
      console.log(`[NETWORK_ERROR] HTTP ${response.status} -> ${r.url}`);
    if (duration > 3000)
      console.log(`[FINDING] SLOW_REQUEST: ${r.url} took ${duration}ms`);
  });
  cdp.on("Network.loadingFailed", ({ requestId, errorText, blockedReason }) => {
    const r = requests.get(requestId);
    console.log(
      `[NETWORK_FAILED] ${r?.url ?? "unknown"}: ${blockedReason ? `blocked:${blockedReason}` : errorText}`,
    );
  });
  ...
  cdp.on("Log.entryAdded", ({ entry }) => {
    if (entry.level === "error" || entry.level === "warning")
      console.log(
        `[LOG:${entry.level.toUpperCase()}] [${entry.source}] ${entry.text}${entry.url ? ` @ ${entry.url}:${entry.lineNumber}` : ""}`,
      );
  });
  ...
  cdp.addReasoningStep?.({
    ...
    nextAction:
      requests.size === 0
        ? "Consider navigation or explicit trigger on same tab"
        : "Drill into failing or slow requests",
  });
}
