...
export async function run(cdp) {
  ...
  cdp.on(
    ...
    ({ requestId, request, wallTime, timestamp, initiator, type }) => {
  ...
  cdp.on("Network.responseReceived", ({ requestId, response, type }) => {
    ...
    if (response.status >= 400) {
      ...
      ms >= SLOW_MS &&
      events.filter((e) => e.kind === "slow-request").length < MAX_STDOUT_ITEMS
    ) {
      console.log(
  ...
  console.log(
    `[METRIC] live-monitor target="${cdp.targetInfo.url}" durationMs=${MONITOR_MS}`,
  );
