...
if (!harArg || hasFlag("--help")) {
  console.error(
    `Usage: node har-pager.mjs <file.har> [--page 1] [--page-size 25] [--filter all|failures|slow|domain:<host>] [--kind <type>] [--status <code>] [--url-regex <re>] [--min-ms 1000] [--format text|json]\n\nReads a HAR 1.2 file and prints one compact page for agent review.`,
  );
...
const pageSize = Math.max(
  1,
  Math.min(200, Number.parseInt(getArg("--page-size", "25"), 10)),
);
...
try {
  ...
  throw new Error(`Invalid HAR JSON: ${harPath}`, { cause: error });
}
...
const aggregate = {
  ...
  counts: {
    failures: entries.filter(
      (entry, index) => compactEntry(entry, index).failed,
    ).length,
    slow: entries.filter((entry) => (entry.time ?? 0) >= minMs).length,
    hosts: new Set(
      entries.map((entry) => hostOf(entry.request?.url)).filter(Boolean),
    ).size,
  },
  pageRows,
};
...
if (format === "json") {
  ...
  console.log(
    `[HAR_PAGE] filter=${filter} page=${page}/${totalPages} pageSize=${pageSize} rows=${rows.length} allEntries=${entries.length}`,
  );
  console.log(
    `[HAR_METRIC] failures=${aggregate.counts.failures} slowOver${minMs}ms=${aggregate.counts.slow} hosts=${aggregate.counts.hosts}`,
  );
  for (const row of pageRows) {
    const status = row.failed ? "FAIL" : "OK";
    console.log(
      `[HAR_ENTRY] #${row.index} ${status} ${row.status} ${row.method} ${row.ms}ms ${row.type} ${row.url}`,
    );
    if (row.errorText || row.blockedReason) {
      console.log(
        `[HAR_DETAIL] #${row.index} error=${row.errorText ?? ""} blocked=${row.blockedReason ?? ""}`,
      );
    }
  }
  if (page < totalPages)
    console.log(
      `[HAR_NEXT] node ${process.argv[1]} ${harPath} --filter ${filter} --page ${page + 1} --page-size ${pageSize} --min-ms ${minMs}`,
    );
}
