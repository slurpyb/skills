...
if (!inPath || hasFlag("--help")) {
  console.error(
    "Usage: node har-redact.mjs <in.har> [--out <out.har>] [--strip-bodies]",
  );
  process.exit(inPath ? 0 : 1);
}

const outPath = resolve(
  getArg("--out", inPath.replace(/\.har$/i, "") + ".redacted.har"),
);
...
let har;
try {
  har = JSON.parse(readFileSync(inPath, "utf8"));
} catch (error) {
  throw new Error(`Invalid HAR JSON: ${inPath}`, { cause: error });
}
