...
if (!ACK)
  fail(
    "Refusing: pass --i-understand-secrets after user approval (CDP can read session secrets).",
  );
if (!TO_PORT && !EXPORT_STATE)
  fail("Need --to-port and/or --export-storage-state.");
const sources = [FROM_PORT, FROM_PROFILE, FROM_STATE].filter(Boolean);
if (sources.length !== 1)
  fail(
    "Pick exactly one of --from-port, --from-profile, --from-storage-state.",
  );
...
if (!Number.isFinite(nodeMajor) || nodeMajor < 22 || !WS) {
  fail(
    `Node.js 22+ required (you have ${process.versions.node}). Native WebSocket is unavailable.`,
  );
}
...
async function connectPort(port) {
  ...
  if (!version.webSocketDebuggerUrl)
    throw new Error(`No browser WS on port ${port}`);
...
function readStorageState(path) {
  ...
  try {
    ...
    throw new Error(`Invalid storage state JSON: ${path}`, { cause: error });
  }
  const list = Array.isArray(raw) ? raw : raw.cookies || [];
  if (!Array.isArray(list))
    fail("storage state must be {cookies:[...]} or an array");
  return list;
}
...
function launchProfileSource(profile, port) {
  return new Promise((resolveLaunch, rejectLaunch) => {
    ...
    console.error(
      `[COOKIE_BRIDGE] Launching profile=${profile} on port=${port} (Chrome must not hold this profile lock)`,
    );
    ...
    child.on("exit", (code) => {
      if (code !== 0) {
        rejectLaunch(
          new Error(`open-browser failed (exit ${code}): ${out.slice(-500)}`),
        );
        return;
      }
      resolveLaunch(out);
...
async function cleanupSourcePort(port) {
  ...
  await new Promise((resolveCleanup) => {
    const child = spawn(
      process.execPath,
      [openBrowser, "--port", String(port), "--cleanup"],
      {
        stdio: "ignore",
      },
    );
...
async function main() {
  ...
  if (platform() === "darwin" && FROM_PROFILE) {
    console.log(
      "[FINDING] macOS: quit Chrome fully before --from-profile, or use --from-port / --from-storage-state",
    );
  }
  ...
  try {
    if (FROM_STATE) {
      if (!existsSync(FROM_STATE))
        fail(`storage state not found: ${FROM_STATE}`);
      ...
      console.log(
        `[METRIC] loaded from-profile=${FROM_PROFILE} source-port=${SOURCE_PORT}`,
      );
    }
    ...
    console.log(
      `[METRIC] cookies=${metas.length} domains=${new Set(metas.map((m) => m.domain)).size}`,
    );
    console.log(
      `[FINDING] cookie-names=${metas
        .map((m) => m.name)
        .slice(0, 40)
        .join(",")}${metas.length > 40 ? ",…" : ""}`,
    );

    if (EXPORT_STATE) {
      writeStorageState(EXPORT_STATE, cookies);
      console.log(
        `[ARTIFACT] storage-state ${resolve(EXPORT_STATE)} mode=0600`,
      );
    }
    ...
    if (TO_PORT) {
      ...
      console.log(
        "[REASON] navigate/target app on to-port; do not print cookie values",
      );
    }
  } finally {
    if (launchedSource) {
